...
public class AddWishlistScreen implements Initializable {
    ...
    public void initData(User currUser, UserCatalogue uc) {
    ...
    public ObservableList<Item> getItems() {
        ...
        for (Item item : allItems) {
            if ((!((Trader)currUser).getWishList().getInv().contains(item)) &&
                    (!((Trader)currUser).getInventory().getInv().contains(item))) {
                items.add(item);
            }
        }
